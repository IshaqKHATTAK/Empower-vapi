...
const Streaming: React.FC<HelloWorldProps> = ({name}) => {
    // use 'useState' to add state to your component
    ...
    // 'useEffect' to perform side effects in your component
    useEffect(() => {
        if (times > 0) {
            document.title = `${name} clicked ${times} times`;
        }
    }, [times, name]); // re-run the effect whenever `times` or `name` changes

    return (
        <div>
          <h1>Hello, {name}</h1>
          <p>{name} clicked {times} times</p>
